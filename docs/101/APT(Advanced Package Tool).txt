*APT(Advanced Package Tool)*
	- APT là công cụ quản lý các gói cấp cao của các hệ điều hành Ubuntu và Debian, là một trong những công cụ
quản lý packages cấp cao phổ biến nhất hiện nay.
	- Có thể sử dụng APT để cài đặt các ứng dụng và các dependencies của ứng dụng.
	- Xóa ứng dụng(Remove applications)
	- Cập nhật(Updates) và nâng cấp(Upgrade) phiên bản của các gói phần mềm đã được cài đặt 
trong hệ điều hành lên phiên bản mới nhất.
